-- 历史数据库结构（迁移测试使用，最新结构见 schema_v015.sql）：schema v12（PRAGMA user_version = 12）。
...
CREATE TABLE raw_records (
    ...
    block_number TEXT, -- 所属区块高度（十进制 u64）；无链上位置时为 NULL，如 Feed 观察记录
...
CREATE TABLE source_cursors (
    chain_id TEXT NOT NULL, -- 链 ID，以十进制 u64 文本保存
    source TEXT NOT NULL, -- 数据源标识，如 rpc、feed
    data BLOB NOT NULL, -- SourceCursor 的 JSON 字节，包含 next_block 和 last_block_hash
...
CREATE TABLE ingest_gaps (
    chain_id TEXT NOT NULL, -- 链 ID，以十进制 u64 文本保存
    source TEXT NOT NULL, -- 数据源标识，如 rpc、feed
    block_number TEXT NOT NULL, -- 区块高度，以十进制 u64 文本保存
