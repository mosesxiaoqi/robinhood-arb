...
状态：执行计划，所有任务均未执行。本文不会创建程序文件，也不表示线上协议已核验。
...
4. 若目录已是 Git 仓库，仅提交该任务文件；当前目录没有 Git，不执行必然失败的提交，也不自动初始化仓库。
...
- [ ] 从架构引用的官方页面核对目标网络，记录 URL、查询日期、链 ID、RPC 和可查证的 Feed 信息；主网与测试网分列，不混用。
- [ ] 对选定公开 RPC 执行只读 `eth_chainId`、`eth_getBlockByNumber`、区块日志和历史状态探测，保存脱敏请求与响应片段、区块哈希、错误码。
- [ ] 分类记录 HTTP/订阅/历史状态/Feed 是否支持；缺失能力写明“不可用”及影响。无公开 Feed 证据时不猜地址。
- [ ] 验收：链 ID 与官方记录一致，至少一个区块可由哈希交叉核对；每项能力都有证据或明确失败原因。
